...
const translateSRTFiles = async pathFile => {
    ...
    const translatedFilename = './teste.srt'.replace(`-${SOURCE_LANGUAGE}`, `-${TARGET_LANGUAGE}`);
    let content;
    const data = fs.readFileSync(pathFile)
    //"./[Erai-raws] 100-man no Inochi no Ue ni Ore wa Tatte Iru - 11 [720p][Multiple Subtitle].txt"
    return await Promise.all([data,  new Promise(function(resolve){
        setTimeout(function() { resolve('I did it'); }, 5000);
      })]) 
      .then(async lol => {
                ...
                return Promise.all(promises)
                    .then(results => {
                        ...
                        content.requestLines.forEach((position, i) => {
                            if (translations[i]) {
                                content.output[position] = translations[i].replace(/\s+(?=[^{\}]*\})/g,'').replace(/\\\sn?N?/g,'\\N').trim();
                            }
                        });
                    })
                    .catch(err => {
...
translateSRTFiles("./[Erai-raws] 100-man no Inochi no Ue ni Ore wa Tatte Iru - 11 [720p][Multiple Subtitle].srt")
